...
const ProgramsDashboard = () => {
  ...
  const fetchDashboard = async () => {
    try {
      setLoading(true)
      const res = await programsService.getDashboard()
  ...
  if (loading) {
    return <div className="pg-page"><div className="pg-loading">Loading dashboard...</div></div>
  }
  ...
  if (error) {
    return (
      <div className="pg-page">
        ...
        <AppButton onClick={fetchDashboard}>Retry</AppButton>
      </div>
    )
  }
  ...
  const columns = [
    ...
    { header: 'Submission Date', accessor: 'submissionDate' },
    {
      ...
      render: (row) => <span className="pg-pill pg-pill-purple">{row.status || row.timesheetStatus}</span>,
    },
    {
      header: 'Contract Status',
      accessor: 'contractStatus',
      render: (row) => {
        const cs = row.contractStatus || ''
        const key = cs.toLowerCase().replace(/\s+/g, '')
        const className =
          key === 'active'
            ? 'pg-pill-green'
            : key === 'onpip'
            ...
            ? 'pg-pill-red'
            : 'pg-pill-gray'
        return <span className={`pg-pill ${className}`}>{cs || 'Active'}</span>
      },
    },
    {
      header: 'Action',
    ...
    <div className="pg-page">
      ...
      <div className="pg-overview-grid">
        <div className="pg-card pg-metric-card">
          ...
          <div className="pg-metric-value">{pendingTimesheets}</div>
        ...
        <div className="pg-card pg-metric-card">
          ...
          <div className="pg-metric-value">{approvedThisMonth}</div>
          <div className="pg-metric-foot">{currentMonth}</div>
        </div>
        ...
        <div className="pg-card pg-metric-card">
          ...
          <div className="pg-metric-value">{pipRecommended}</div>
        ...
        <div className="pg-overview-right">
          <div className="pg-card pg-wide-card">
            ...
            <div className="pg-wide-value">{contractReviewRequired}</div>
          </div>
          ...
          <div className="pg-card pg-wide-card">
            ...
            <div className="pg-wide-value">{superviseesCount}</div>
          </div>
        </div>
      </div>
      ...
      <div className="pg-card pg-table-card">
        ...
        <AppTable
          ...
          emptyMessage="No active timesheets"
        />
      </div>
    </div>
  )
