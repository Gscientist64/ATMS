...
const EcewsSupervisorDashboard = () => {
  ...
  const fetchDashboard = async () => {
    try {
      const res = await ecewsSupervisorService.getDashboard()
  ...
  const fetchTimesheets = async () => {
    try {
      const res = await ecewsSupervisorService.getTimesheetsForReview('pending')
      setTimesheets(Array.isArray(res) ? res : (res.data || []))
    } catch (err) {
  ...
  const approvedCount = dashboardData?.approvedCount ?? 0
  const activePips = dashboardData?.activePips ?? 0
  const totalStaff = dashboardData?.totalStaff ?? 0
  const currentMonth = new Date().toLocaleString('default', { month: 'long', year: 'numeric' })

  const overviewCards = [
    { id: 'pending', label: 'Pending Timesheets', value: pendingCount, detail: 'Awaiting your review', iconBg: '#FFF7E6', iconColor: '#F59E0B' },
    { id: 'approved', label: 'Approved this month', value: approvedCount, detail: currentMonth, iconBg: '#ECFDF3', iconColor: '#16A34A' },
    { id: 'pip', label: 'PIP Advised', value: activePips, detail: 'Active PIPs', iconBg: '#FEF2F2', iconColor: '#E11D48' },
    { id: 'contract', label: 'Contract Review Required', value: 0, detail: '', iconBg: '#EEF2FF', iconColor: '#4B5563' },
    { id: 'supervisees', label: 'Supervisees', value: totalStaff, detail: '', iconBg: '#E5E7EB', iconColor: '#4B5563' },
  ]

  const mainOverviewCards = overviewCards.slice(0, 3)
  const sideOverviewCards = overviewCards.slice(3)

  const activeTimesheets = timesheets.length > 0 ? timesheets.map(t => ({
    id: t.id,
